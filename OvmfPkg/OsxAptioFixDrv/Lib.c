...
FileDevicePathToText(EFI_DEVICE_PATH_PROTOCOL *FilePathProto)
{
		...
		if (FilePathProto->Type == MEDIA_DEVICE_PATH && FilePathProto->SubType == MEDIA_FILEPATH_DP) {
			...
			if (SizeAll + Size < 256) {
				if (SizeAll > 0 && FilePathText[SizeAll / 2 - 2] != L'\\') {
					StrCatS(FilePathText, sizeof(FilePathText)/sizeof(FilePathText[0]), L"\\");
				}
				StrCatS(FilePathText, sizeof(FilePathText)/sizeof(FilePathText[0]), FilePath->PathName);
	...
	if (Size > 2) {
		...
		if (Status == EFI_SUCCESS) {
			StrCpyS(OutFilePathText, Size / sizeof(FilePathText[0]), FilePathText);
		} else {
			OutFilePathText = NULL;
		}
	}
